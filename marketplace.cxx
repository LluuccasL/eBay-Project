...
bool Marketplace::placeBid(int bidderID, int listingID, double amount)
{
    ...
    logEvent("bid_placed", "User " + to_string(bidderID) +
             " bid " + to_string(amount) +
             " on listing " + to_string(listingID));

    int winner = listing->getHighestBidder();
    double price = listing->getCurrentPrice();
    int lid = listing->getID();
    string lname = listing->getName();

    // If buy-now price reached, close auction immediately
    if (price >= listing->getBuyNowPrice())
    {
        ...
        // bidders
        for (int bidderId : listing->getBidderIDs())
        ...
        logEvent(
            "auction_closed",
            "Listing " + to_string(lid) +
            " sold to user " + to_string(winner) +
            " at " + to_string(price)
        );
    }
...
void Marketplace::showMarketplace()
{
	...
	int bidder = l->getHighestBidder();
        cout << "ID: " << l->getID()
             << " | " << l->getName()
             << " | Category: " << l->getCategory()
             << " | Price: $" << l->getCurrentPrice()
	     << " | Buy Now: $" << l->getBuyNowPrice()
             << " | Highest Bidder: ";
	if (bidder == -1)
		cout << "None";
	else
		cout << bidder;
	cout<< " | Expires: t=" << l->getEndTime() << endl;
    }
}
...
void Marketplace::closeExpiredAuctions()
{
    for (auto &l : listings)
    {
        if (l.getStatus() == ListingStatus::ACTIVE && !l.isActive(simClock))
        {
